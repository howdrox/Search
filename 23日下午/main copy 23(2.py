...
class Game:
    def __init__(self, height, width):
        ...
        #     p.show()
        self.muzzle = muzzle(self,Person)
       
        self.root.bind("<KeyPress>", lambda e: self.persons[0].key_press(e.keysym),lambda f:self.muzzle.move_muzzle(f.keysym))
        self.root.bind("<KeyRelease>", lambda e: self.persons[0].key_release(e.keysym),lambda f:self.muzzle.move_muzzle(f.keysym))
        self.root.mainloop()
...
class muzzle:
    def __init__(self,game,person):
        ...
        self.game = game
        self.muzzle = self.game.c.create_oval(20,20,40,40,fill='yellow', edge = None)#initialise
        self.person = person
        self.j_muzzle = 0
        self.i_muzzle = 0
    ...
    def check_muzzle(self, j_test, i_test):#just for beauty
        return self.person.check_movement(j_test, i_test)
    ...
    def move_muzzle(self):
        ...
        i_muzzle_test = self.person.i + self.i_muzzle
        j_muzzle_test = self.person.j + self.j_muzzle
        if self.check_muzzle(j_muzzle_test, i_muzzle_test):
            r_size = self.game.r_size
            x= i_muzzle_test * r_size
            y= j_muzzle_test * r_size
            self.game.c.coords(self.muzzle,(x+0.42*r_size,y+0.42*r_size
